...
def main():
    ...
    parser.add_argument(
        "--input_json", type=str, default="/workspace/zinc_example_data/zinc_data_converted.json", required=False, help="input_file_json_path"
    )
    parser.add_argument(
        "--output_json", type=str, default="/workspace/results/megamolbart_evaluation.json",required=False
    )
    parser.add_argument(
    ...
    with open(args.input_json, "r") as f:
        data = json.load(f)
        smiles_list = [entry["smiles"] for entry in data]
    # evaluation
    results=[]
    for molecule in tqdm(smiles_list):
        request = {
            "prompt": molecule,
        ...
        print("***************************")
        r = (input_mol==recon_mol)
        # append data into dictionary
        result = {
            "input molecule": input_mol,
            "output molecule": recon_mol,
            "result": r
        }
